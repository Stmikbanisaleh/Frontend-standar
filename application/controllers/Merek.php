...
class Merek extends CI_Controller
{
	...
	public function save()
	{
		...
		foreach ($dokmerek as $dm) {
			...
			$config['file_name']          = $dm['PENAMAAN_FILE'] . '_' . $ipmancode;
			...
			if (!empty($_FILES['dokumen' . $i]['name'])) {
				...
				$filename = $dm['PENAMAAN_FILE'] . '_' . $ipmancode;
				$size = '';
				$type = '';
				$jenisdok = $dm['ID'];
			}
	...
	public function update()
	{
		...
		$i = 1;
		foreach ($dokmerek as $dm) {
			$config['file_name']          = $dm['PENAMAAN_FILE'] . '_' . $ipman;
			$config['upload_path']          = './assets/dokumen/dokumen_merek/';
			$config['allowed_types']        = 'doc|docx|pdf';
			$config['overwrite']        = TRUE;
			...
			$dokumen[$i] = array($filename[$i], $size[$i], $type[$i], '1', $jenisdok[$i], $dateinput[$i], $userinput[$i], $dateubah[$i], $userubah[$i]);
			$i++;
		}
		switch ($jumlahdok) {
			case "1":
		...
		if ($this->db->update('msmerek', $data)) {
			$this->db->delete('msdokumen', array('NOMOR_PENDAFTAR' => $this->input->post('ipman_code')));
	...
	public function save_verifikasi()
	{
		...
		$data = [
			'TAHUN_PENDAFTARAN' => $this->input->post('thn_pendaftaran'),

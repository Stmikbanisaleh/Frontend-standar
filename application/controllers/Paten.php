...
class Paten extends CI_Controller
{
	...
	public function save()
	{
		...
		foreach ($dokpaten as $dp) {
			...
			$config['file_name']          = $dp['PENAMAAN_FILE'] . '_' . $ipmancode;
			...
			if (!empty($_FILES['dokumen' . $i]['name'])) {
				...
				$filename = $dp['PENAMAAN_FILE'] . '_' . $ipmancode;
				$size = '';
				$type = '';
				$jenisdok = $dp['ID'];
			}
		...
		$configab['file_name']          = 'abstrak_' . $ipmancode;
		...
		$cgambar['file_name']          = 'gambar_paten_' . $ipmancode;
	...
	public function update()
	{
		...
		foreach ($dokpaten as $dp) {
			$versi = $dp['REV'] + 1;
			if ($dp['SIZE']) {
				$config['file_name']          = $dp['PENAMAAN_FILE'] . '_' . $ipman . '_v' . $versi;
				$config['upload_path']          = './assets/dokumen/dokumen_paten/';
				$config['allowed_types']        = 'doc|docx|pdf';
			} else {
				$config['file_name']          = $dp['PENAMAAN_FILE'] . '_' . $ipman;
				$config['upload_path']          = './assets/dokumen/dokumen_paten/';
				$config['allowed_types']        = 'doc|docx|pdf';
			}
			...
			$dokumen[$i] = array($filename[$i], $size[$i], $type[$i], $rev[$i], '1', $jenisdok[$i], $dateinput[$i], $userinput[$i], $dateubah[$i], $userubah[$i]);
			$i++;
		}
		switch ($jumlahdok) {
			case "1":
		...
		$data = [
			'JUDUL' => htmlspecialchars($this->input->post('judul', true)),
			'ABSTRAK' => htmlspecialchars($this->input->post('abstrak', true)),
			'BIDANG_INVENSI' => htmlspecialchars($this->input->post('bidang_invensi', true)),
			'UNIT_KERJA' => $this->input->post('unit_kerja'),
			'STATUS' => 19,
			'NO_HANDPHONE' => $this->input->post('no_handphone'),
		...
		if ($this->db->update('mspaten', $data)) {
			$this->db->delete('msdokumen', array('NOMOR_PENDAFTAR' => $this->input->post('ipman_code'), 'REV' => 0));
